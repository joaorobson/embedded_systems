 ...
 *
 */
int8_t get_sensor_data_forced_mode(struct bme280_dev *dev, float *temp, float *hum);
...
void get_bme280_temperature(float* temperature, float *humidity, char* device, struct identifier id, struct bme280_dev dev)
{
    ...
    //    exit(1);
    //}
    rslt = get_sensor_data_forced_mode(&dev, temperature, humidity);
    if (rslt != BME280_OK)
    {
 ...
 * @brief This API used to print the sensor temperature, pressure and humidity data.
 */
float format_sensor_data(struct bme280_data *comp_data)
{
    float temp;
...
int8_t get_sensor_data_forced_mode(struct bme280_dev *dev, float *temp, float *hum)
{
    ...
    settings_sel = BME280_OSR_TEMP_SEL | BME280_FILTER_SEL | BME280_OSR_HUM_SEL;
