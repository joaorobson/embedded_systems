...
extern TaskHandle_t suspend_LED_loop;
extern TaskHandle_t suspend_HTTPRequest;
                          ...
                          int32_t event_id, void *event_data)
{
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        if (suspend_HTTPRequest != NULL)
        {
            vTaskSuspend(suspend_HTTPRequest);
        }

        eTaskState led_loop_state = eTaskGetState(suspend_LED_loop);
        if (led_loop_state == eSuspended)
        {
            ESP_LOGI(TAG, "Ativando LED piscando em loop");
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        ESP_LOGI(TAG, "Suspendendo LED piscando em loop");
        vTaskSuspend(suspend_LED_loop);

        if (suspend_HTTPRequest != NULL)
        {
            vTaskResume(suspend_HTTPRequest);
        }
        else
        {
